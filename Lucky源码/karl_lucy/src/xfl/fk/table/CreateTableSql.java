...
public class CreateTableSql {
	...
	private static LuckyConfig lucy=LuckyConfig.getConfig();
	...
	public static String getCreateTable(Class clzz) {
		...
		for (int i = 0; i < fields.length; i++) {
			if (i < fields.length - 1) {
				if (fields[i].getName().equals(AnnotationCgf.getAnnCfg().getId(clzz)))
					sql += fields[i].getName() + " " + tych.toMysql(fields[i].getType().toString()) + "("+lucy.fieldLength("FieldLength")+") "
					...
					sql += fields[i].getName() + " " + tych.toMysql(fields[i].getType().toString()) + "  DEFAULT NULL,";
				}
			} else {
				if (fields[i].getName().equals(AnnotationCgf.getAnnCfg().getId(clzz)))
					sql += fields[i].getName() + " " + tych.toMysql(fields[i].getType().toString()) + "("+lucy.fieldLength("FieldLength")+") "
	...
	public static List<String> getForeignKey(Class clzz) {
		...
		if (keys == null) {
			...
			Class[] cs = new Class[keys.length];
			cs = AnnotationCgf.getAnnCfg().getMainTables(clzz);
